...
Settings_Mycrm_Detail_Js('Settings_Webforms_Detail_Js', {
    ...
    deleteRecord : function(deleteRecordActionUrl) {
		...
		Mycrm_Helper_Js.showConfirmationBox({'message' : message}).then(function(data) {
				AppConnector.request(deleteRecordActionUrl).then(
				function(data){
					if(data.success == true){
						window.location.href = data.result;
					}else{
						Mycrm_Helper_Js.showPnotify(data.error.message);
					}
				});
			},
			function(error, err){
	...
	registerEvents : function() {
		this._super();
		Mycrm_Helper_Js.showHorizontalTopScrollBar();
	}
})